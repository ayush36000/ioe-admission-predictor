...
def calculate_cutoff():
    ...
    for col_program in CollegeProgramCollection:
        ...
        raw_query = "SELECT max(EntranceRank) as cutoff FROM `entrance_student` as `es` INNER JOIN admisssion_list_entrance_student as `ad_es` ON ad_es.EntranceStudentId=es.EntranceStudentID INNER JOIN admission_list_college_record as `ad_col` ON ad_col.AdmissionListCollegeId = ad_es.AdmissionListCollegeId WHERE ad_col.CollegeId = {} and ad_col.ProgramId = {};".format(
            col_program.college_id, col_program.program_id)
        cutoff = raw_query.cutoff

        CollegeProgramCollection.cutoff = cutoff
        cutoff.save()
